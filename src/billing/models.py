...
from .signal import membership_dates_update
...
def update_membership_dates(sender, new_date_start, new_end_date, **kwargs):
	membership = sender
...
class  TransactionManager(models.Manager):
	def create_new(self, user, transaction_id, amount, card_type, success=None, transaction_status=None, last_four=None):
		...
		new_order_id = "%s%s%s" %(transaction_id[:2],random.randint(1, 1738),transaction_id[2:])
...
class Transaction(models.Model):
	...
	transaction_id = models.CharField(max_length=128)#braintree or stripe
